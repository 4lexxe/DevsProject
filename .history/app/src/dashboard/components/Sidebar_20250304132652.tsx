...
const Sidebar: React.FC = () => {
  ...
  return (
    <>
      {/* Botón móvil - ahora visible en todas las resoluciones */}
      <button
        onClick={toggleMobileSidebar}
        className="fixed top-4 left-4 z-50 flex items-center justify-center w-11 h-11 rounded-md bg-white shadow-md text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors"
      ...
      {/* Sidebar móvil - ahora es el único sidebar y se muestra en todas las resoluciones */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 bg-white shadow-xl transform transition-transform ${
          mobileOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
        style={{ 
          width: collapsed ? '80px' : '288px', // Width fijo: 80px colapsado, 288px expandido 
      ...
      {/* Eliminar el sidebar desktop para usar solo una versión */}
    </>
  );
};
