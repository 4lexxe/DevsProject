...
const Sidebar: React.FC = () => {
  ...
  const sidebarContent = (
    <>
      {/* Cabecera fija con botón toggle centrado cuando está colapsado */}
      <div className={`flex-shrink-0 flex h-16 border-b border-gray-200 ${
      ...
      {/* Área de navegación con scroll - ajustado para mantener consistencia */}
      <div className="flex-grow overflow-y-auto">
        <nav className={`space-y-1 ${collapsed ? 'p-4' : 'p-3'}`}>
          {navItems.map((item, index) => (
            item.isHeader ? (
              // Renderizar encabezado de sección - centrado en modo colapsado
              <div key={index} className={`mt-4 mb-2 ${
                ...
                {collapsed && (
                  <div className="border-b border-gray-200 w-4/5 mb-1"></div>
                )}
              </div>
            ) : (
              // Renderizar elemento de navegación normal - iconos centrados en modo colapsado
              <a
                key={index}
                href="#"
                className={`flex items-center rounded-lg transition-colors ${
                  collapsed 
                    ? 'py-3 px-2 flex-col justify-center' 
                    : 'py-2 px-3'
                } ${
                  item.active 
              ...
              >
                <div className={`flex items-center justify-center ${collapsed ? 'w-9 mb-1' : 'w-7'}`}>
                  {item.icon(iconSize)}
                </div>
                {collapsed && (
                  <span className="text-[10px] text-center font-medium overflow-hidden text-ellipsis w-full">
                    {item.label}
                  </span>
                )}
      ...
      {/* Footer fijo - centrado en modo colapsado */}
      <div className="flex-shrink-0 p-3 border-t border-gray-200">
        <a
          href="#"
          className={`flex rounded-lg transition-colors ${
            collapsed 
              ? 'py-3 px-2 flex-col items-center justify-center' 
        ...
        >
          ...
          {collapsed && (
            <span className="text-[10px] text-center font-medium">Logout</span>
          )}
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{ width: collapsed ? '120px' : '280px' }}
      >
        {sidebarContent}
      </aside>
