...
const ResourceDetailsPage: React.FC = () => {
  ...
  const fetchResourceDetails = async () => {
    try {
      ...
      // Obtener SOLO el recurso, que ya incluye la información del usuario
      const resourceData = await ResourceService.getResourceById(parseInt(id));
      
      // El backend devuelve el recurso directamente, no necesitamos información adicional del usuario
  ...
  // Helper para obtener el nombre del autor usando la información que ya viene en el recurso
            ...
            <div className="flex items-center text-gray-600 mb-4">
              <div className="flex items-center">
                <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white text-sm font-medium mr-3">
                  {resource.User ? getAuthorName(resource.User).charAt(0).toUpperCase() : 'U'}
                </div>
                <span>Por: {resource.User ? getAuthorName(resource.User) : 'Usuario desconocido'}</span>
              </div>
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <dd className="text-sm text-gray-900">
                {resource.User ? getAuthorName(resource.User) : 'Usuario desconocido'}
              </dd>
            </div>
            <div>
