...
const ResourceDetailsPage: React.FC = () => {
  ...
  const fetchResourceDetails = async () => {
    try {
      ...
      // SOLO obtener el recurso usando el endpoint público - ya incluye información del usuario
      const resourceData = await ResourceService.getResourceById(parseInt(id));
      
      console.log('📦 Recurso obtenido:', resourceData);
      
      // El backend devuelve el recurso con la información del usuario incluida
  ...
  // Helper para obtener el nombre del autor SOLO con la información incluida en el recurso
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mb-4 p-4 bg-yellow-100 border rounded">
            <p><strong>Debug:</strong> Resource ID: {resource.id}</p>
            <p><strong>User info:</strong> {resource.User ? 'Incluido' : 'No incluido'}</p>
            <p><strong>User name:</strong> {resource.User ? getAuthorName(resource.User) : 'N/A'}</p>
          </div>
        )}
