...
const ResourceDetailsPage: React.FC = () => {
  ...
  const fetchResourceDetails = async () => {
    try {
      ...
      console.log('🔍 Obteniendo SOLO el recurso con ID:', id);
      
      // ABSOLULTAMENTE NO usar UserService aquí - solo ResourceService
  ...
  // Helper para obtener el nombre del autor usando SOLO la información del recurso
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Debug para verificar que NO se hacen llamadas adicionales */}
        {process.env.NODE_ENV === 'development' && (
          <div className="mb-4 p-3 bg-green-100 border border-green-400 rounded">
            <p className="text-green-800 text-sm">
              ✅ Usando SOLO ResourceService - NO UserService
            </p>
            <p className="text-green-700 text-xs">
              Usuario incluido: {resource.User ? 'SÍ' : 'NO'} | 
              Nombre: {resource.User ? getAuthorName(resource.User) : 'N/A'}
            </p>
          </div>
        )}
        ...
        {resource.coverImage && (
          <div className="mb-8">
            <img 
              ...
              onError={(e) => {
                console.error('Error loading image:', resource.coverImage);
