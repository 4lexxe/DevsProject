...
const Comment: React.FC<CommentProps> = ({ resourceId }) => {
  const { user, canModifyComment, hasPermission, debugPermissions } = usePermissions();
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('💬 Comment Component Debug:', {
        user: user,
        canComment: hasPermission('comment:resources'),
    ...
    <div className="space-y-6">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="p-3 bg-blue-50 border border-blue-200 rounded-md">
          <p className="text-blue-800 text-sm font-medium">🔍 Debug Comentarios:</p>
          <ul className="text-xs text-blue-700 mt-1">
            <li>Usuario autenticado: {user ? '✅ Sí' : '❌ No'}</li>
            <li>Rol: {user?.role?.name || 'N/A'}</li>
            <li>Puede comentar: {canComment ? '✅ Sí' : '❌ No'}</li>
            <li>Permisos: {user?.role?.permissions?.join(', ') || 'Ninguno'}</li>
          </ul>
        </div>
      )}
