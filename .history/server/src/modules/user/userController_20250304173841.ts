...
export class UserController {
  ...
  static async getUserPermissions(req: Request, res: Response): Promise<void> {
    try {
      ...
      const effectivePermissions = allPermissions.filter(p => !blockedPermissionIds.has(p.id));
      
      // Devolver la información de permisos
      res.json({
        userId: user.id,
        username: user.username || user.email,
        roleName: user.Role?.name,
        permissions: {
          rolePermissions: rolePermissions.map(p => ({ id: p.id, name: p.name, description: p.description })),
          customPermissions: customPermissions.map(p => ({ id: p.id, name: p.name, description: p.description })),
          blockedPermissions: blockedPermissions.map(p => ({ id: p.id, name: p.name, description: p.description })),
          effectivePermissions: effectivePermissions.map(p => ({ id: p.id, name: p.name, description: p.description }))
        }
      });
