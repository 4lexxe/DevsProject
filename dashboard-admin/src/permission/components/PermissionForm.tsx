...
const initialFormState: PermissionCreateRequest = {
  name: '',
  description: ''
};
...
const PermissionForm: React.FC<PermissionFormProps> = ({
  ...
  const [formData, setFormData] = useState<PermissionCreateRequest>(initialFormState);
  ...
  const validate = (): boolean => {
    ...
    if (!formData.name.trim()) {
      ...
      newErrors.name = 'El nombre solo puede contener letras minúsculas, números, guiones, dos puntos y guiones bajos';
    }
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        <div className="space-y-2">
          ...
          <input
            ...
            type="text"
            value={formData.name}
            onChange={(e) => handleChange('name', e.target.value.toLowerCase().trim())}
            placeholder="Ej: read:users, manage:courses, etc."
            required
            className={`w-full px-4 py-2.5 text-sm border rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-100 bg-white dark:bg-gray-700/50 dark:text-white dark:border-gray-600 font-mono ${
              errors.name
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Formato: letras minúsculas, números, guiones, dos puntos y guiones bajos (ej: read:users)
          </p>
        </div>
