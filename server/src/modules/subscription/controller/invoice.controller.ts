...
class InvoiceController {
  ...
  static async createInvoiceInDB(invoiceData: any) {
    try {
      ...
      const newInvoice = await Invoice.create({
        ...
        mpSubscriptionId: invoiceData.preapproval_id, // Asumiendo que invoiceData tiene la suscripción
        data: invoiceData,
        issueDate,
        retryAttempts: invoiceData.retry_attempts, // Usar el valor de reintentos si está presente
      });
  ...
  static async updateInvoiceInDB(invoiceData: any, invoiceId: number | string) {
    try {
      ...
      if (!invoice) {
        console.error(
          `No se encontró la factura para actualizar: ID ${invoiceId}`
        );
        return null;
      }
      ...
      invoice.retryAttempts = invoiceData.retry_attempts; // Mantener el valor actual si no se proporciona uno nuevo
  ...
  static downloadInvoicePDF: RequestHandler = async (req, res) => {
    ...
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=factura_${paymentId}.pdf`
      );
