...
if (discordClientId && !/^\d+$/.test(discordClientId)) {
  console.log('   ⚠️  DISCORD_CLIENT_ID no tiene el formato correcto (debe ser numérico)');
  ...
  if (clientIdBigInt > maxSnowflake) {
    ...
    console.log('   ⚠️  Este número es inválido. Verifica que estés copiando el Client ID correcto.');
...
if (discordClientSecret && discordClientSecret.length < 32) {
  console.log('   ⚠️  DISCORD_CLIENT_SECRET parece muy corto (normalmente tiene 32+ caracteres)');
...
if (discordCallbackUrl) {
  try {
    ...
    if (url.protocol !== 'http:' && url.protocol !== 'https:') {
      console.log('   ⚠️  DISCORD_CALLBACK_URL debe usar http:// o https://');
    } else {
