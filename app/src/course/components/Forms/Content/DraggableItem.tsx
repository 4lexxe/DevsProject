...
export default function  DraggableItem({
                           ...
                           onEdit,
                           onDelete,
                       }: { item: IContent; index: number; onEdit: any; onDelete: any }) {
    ...
    const { updateContentPosition } = useContentContext()

    useEffect(() => {

        updateContentPosition(item.id, index + 1 )
    }, []);
        ...
        <GripVertical className="w-6 h-6" />
            ...
            <div className="flex-1 px-4 space-y-2">
                <h5 className="font-semibold text-gray-800 text-lg leading-tight">
                    {index + 1}. {item.contentTextTitle}
                </h5>
                <p className="text-sm text-gray-600 line-clamp-2">{item.contentText}</p>
